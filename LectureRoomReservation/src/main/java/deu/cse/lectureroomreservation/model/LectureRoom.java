    ...
    public boolean occupied(DaysOfWeek day, int period) {
        ...
        if (this.lectureInfo[day.index()] != null) {
            for (String item : this.lectureInfo[day.index()]) {
                String[] data = item.split(",");  // 4,OOD,1분반 --> 교시,교과목,분반
        ...
        for (String item : this.reservationInfo) {
            String[] data = item.split(",");  // 1,1,홍길동  --> 요일,교시,예약자
            if (day.index() == Integer.parseInt(data[0])) {
                if (period ==  Integer.parseInt(data[1])) {
                    occupied = true;
                    break;
                }
            }
