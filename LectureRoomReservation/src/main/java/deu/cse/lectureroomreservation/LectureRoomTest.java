...
public class LectureRoomTest {
    ...
    public static void test2() throws Exception {
        ...
        log.info("916호실: 화요일 1-2교시 사용 가능 = {}\n\n", isAvailable);
        if (isAvailable) {
        ...
        // IEB916호실의 화요일 2,3교시 강의실 예약
        isAvailable = rc.canReserveRoom("IEB916", DaysOfWeek.TUESDAY, 2, 3);
        log.info("916호실: 화요일 2-3교시 사용 가능 = {}\n\n", isAvailable);
        if (isAvailable) {
            boolean successful = rc.makeReservation("IEB916", DaysOfWeek.TUESDAY, 2, 3, "홍길동");
            if (successful) {
                log.info("916호실 화요일 2-3교시 강의실 예약 성공\n\n");
            } else {
                log.info("916호실 화요일 2-3교시 강의실 예약 실패\n\n");
            }
        }
        
        isAvailable = rc.canReserveRoom("IEB918", DaysOfWeek.TUESDAY, 1, 2);
        log.info("918호실: 화요일 1-2교시 사용 가능 = {}\n\n", isAvailable);
        if (isAvailable) {
        ...
        try (InputStream is = LectureRoomTest.class.getResourceAsStream("/room_info.yml")) {
            ...
            if (obj.containsKey(year) && obj.get(year).containsKey(semester)) {
                ...
                for (String roomId : values.keySet()) {
                    log.debug("id = {}, data = {}", roomId, values.get(roomId));
                    List<String>[] data = new ArrayList[7];
                    ...
                    for (String day : info.keySet()) {
                        ...
                        log.debug("day = {}, index = {}", day, index);
                        if (data[index] == null) {
                            data[index] = new ArrayList<>();
                        }
                        for (String hourlyInfo : info.get(day)) {
                            log.debug("hourlyInfo = {}", hourlyInfo);
